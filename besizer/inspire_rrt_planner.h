...
public:
   ...
   // vector<int>   findCellPath(RRT &myRRT, int startCell, int goalCell,vector<vector<float> > gridVal);//使用温度场作为启发因子
    vector<RRT::rrtNode>   findCellPath(RRT &myRRT1,RRT &myRRT2,  int startCell, int goalCell);//使用温度场作为启发因子
    void convertToPlan(vector<RRT::rrtNode> bestPath,std::vector<geometry_msgs::PoseStamped>& plan, geometry_msgs::PoseStamped goal,int goalCell);
    void publishPlan(const std::vector<geometry_msgs::PoseStamped>& path);
    void publishPlan2(const std::vector<geometry_msgs::PoseStamped>& path);
    vector<RRT::rrtNode> bersier(vector<RRT::rrtNode>rrtPath);
    ...
    void generateRandNodes(RRT &myRRT, vector<RRT::rrtNode> &randNodes,int width, int height,float m,float n)//生成一个随机结点数组
    {
    ...
    bool checkConnect(RRT::rrtNode &tempNode1,RRT::rrtNode &tempNode)
    {
        int index;
     
           double distance = sqrt(pow(tempNode.posX-tempNode1.posX,2)+pow(tempNode.posY-tempNode1.posY,2));
            if((distance < rrtStepSize)&&(pointcheck(tempNode,tempNode1)))
            {
                return true;
            }
            else
...
bool checkConnect(RRT &myRRT,RRT::rrtNode &tempNode)
    {
        
     for(int i=0;i<myRRT.getTreeSize();i++)
     {
           double distance = sqrt(pow(tempNode.posX-myRRT.getNode(i).posX,2)+pow(tempNode.posY-myRRT.getNode(i).posY,2));
            if((distance < rrtStepSize)&&(pointcheck(tempNode,myRRT.getNode(i))))
            {
                return true;
            }
            else
